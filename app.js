...
async function analisarPerfil() {
  ...
  if (!texto) { setStatus(status, 'error', 'Cole o texto do perfil antes de analisar.'); return; }
  ...
  const prompt = buildPrompt(texto, vagaAtiva);
  ...
  try {
    ...
    if (!json) throw new Error('Resposta da IA não foi reconhecida. Tente novamente.');
...
function buildPrompt(texto, vaga) {
  return `Você é um sistema especialista em recrutamento e seleção. Analise o perfil do candidato abaixo em relação à vaga fornecida e responda SOMENTE com um JSON válido, sem texto antes ou depois, sem markdown.
...
Retorne EXATAMENTE este JSON (sem comentários, sem markdown):
{
  ...
  "criteriosAtendidos": ["critério 1", "critério 2"],
...
REGRAS IMPORTANTES:
- aderencia: número de 0 a 100 baseado em quão bem o candidato atende os requisitos da JD
- Se o candidato não tem NENHUMA experiência relevante para a vaga, aderencia deve ser entre 0 e 15
- estabilidadeMedia: calcule a média ponderada das notas de cada experiência SIMILAR (ignoradas e experiências não similares não entram no cálculo)
  - nota 1 = até 4 meses; nota 2 = 5 a 11 meses; nota 3 = 1 ano a 3 anos 11 meses; nota 4 = 4 anos ou mais
  - Estágio e Freelance/Autônomo têm peso 0.5; Tempo integral, Contrato têm peso 1.0
- similar: true apenas se a experiência for relevante para a vaga
- ignorada: true se a experiência não tem nenhuma relação com a área da vaga
- dataIncompleta: true se não foi possível identificar o período claramente
- dentroDoRaio: true se a localização do candidato for compatível com a cidade/raio da vaga (se não houver raio definido, sempre true)`;
}
